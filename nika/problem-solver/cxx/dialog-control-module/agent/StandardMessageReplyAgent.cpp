...
ScResult StandardMessageReplyAgent::DoProgram(ScActionInitiatedEvent const & event, ScAction & action)
{
  ...
  auto resolve = [&](char const * sysId) {
    return m_context.ResolveElementSystemIdentifier(sysId, ScType::ConstUnknown);
  };

  ScAddr conceptGenerate = resolve("concept_generate_prompt_message");
  ScAddr nrelRole        = resolve("nrel_role");
  ScAddr nrelDomain      = resolve("nrel_domain");
  ScAddr nrelTask        = resolve("nrel_task");

  ScAddr cMissingTask    = resolve("concept_message_without_task");
  ScAddr cMissingDomain  = resolve("concept_message_without_domain");
  ScAddr cMissingRole    = resolve("concept_message_without_role");
  ScAddr cNotGenerate    = resolve("concept_message_not_generate_prompt");

  auto isMember = [&](ScAddr const & klass) {
    return klass.IsValid() && m_context.CheckConnector(klass, messageNode, ScType::ConstPermPosArc);
  };
  auto ensureClass = [&](ScAddr const & klass) {
    if (klass.IsValid() && !isMember(klass))
      m_context.GenerateConnector(ScType::ConstPermPosArc, klass, messageNode);
  };
  auto removeClass = [&](ScAddr const & klass) {
    if (!klass.IsValid()) return;
    ScIterator3Ptr it = m_context.CreateIterator3(klass, ScType::ConstPermPosArc, messageNode);
    while (it->Next()) m_context.EraseElement(it->Get(1));
  };
  auto hasOutRel = [&](ScAddr const & rel) {
    if (!rel.IsValid()) return false;
    return utils::IteratorUtils::getAnyByOutRelation(&m_context, messageNode, rel).IsValid();
  };

  removeClass(cMissingTask);
  removeClass(cMissingDomain);
  removeClass(cMissingRole);
  removeClass(cNotGenerate);

  if (isMember(conceptGenerate))
  {
    bool const hasRole   = hasOutRel(nrelRole);
    bool const hasDomain = hasOutRel(nrelDomain);
    bool const hasTask   = hasOutRel(nrelTask);

    if (!hasTask)
      ensureClass(cMissingTask);
    else if (!hasDomain)
      ensureClass(cMissingDomain);
    else if (!hasRole)
      ensureClass(cMissingRole);
  }
  else
  {
    ensureClass(cNotGenerate);
  }
